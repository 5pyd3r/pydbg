...
| `detach(pid=None)` | 分离调试器 |
| `terminate_process(exit_code=1)` | 终止被调试进程 |
...
| 方法 | 说明 |
|------|------|
| `read_memory(addr, size)` | 读取进程内存 |
| `write_memory(addr, data)` | 写入进程内存 |
| `query_memory(addr)` | 查询内存区域信息 |
| `protect_memory(addr, size, protect)` | 修改页面保护 |
...
| 方法 | 说明 |
|------|------|
| `set_breakpoint(addr)` | 设置 int3 软件断点 |
