...
构建需要 LLVM/Clang + MSVC Build Tools（提供 lld-link 和 Windows SDK）：

```bat
demo\build_dll.bat
```

> **注意**：Git for Windows 自带的 `link.exe`（Unix 硬链接工具）会覆盖 MSVC 的 `link.exe`，构建脚本使用 `-fuse-ld=lld` 强制 clang 调用 `lld-link`。
